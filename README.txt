Readme.1st for V0.71 of Beebem (see 'changes' file for history)
...
The source files are set up to compile the Microsoft Windows version so
the first thing to do is rename a few files:
  main.h.x11     -> main.h
  main.cc.x11    -> main.cc
  beebwin.h.x11  -> beebwin.h
  beebwin.cc.x11 -> beebwin.cc
  Rename all files with a '.cpp' extension to a '.cc' extension
...
Linux.  You need a lot of processing speed for it.

Now type 'make' - and hope for the best! When it doesn't work look at the
bottom of this file for some hints.

NOTE: The sources contain conditional compilation for the MS Windows version
of Beebem.  The UNIX version of Beebem has not been fully compiled since
these changes were made.  It should still work but I may have broken it (MW)!
...
Serial, ADC, Printer port, keyboard LEDs, sound, tube, light pen, and
